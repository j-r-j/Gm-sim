...
export function simulateLeagueHistory(
  ...
  for (let yearIndex = 0; yearIndex < years; yearIndex++) {
    ...
    // Calculate salary cap for the next year's offseason moves
    // Cap grows at 8% per year from 2024 (matching getFranchiseTagValue behavior)
    const offseasonYear = currentYear + 1;
    const yearsFrom2024 = offseasonYear - 2024;
    const inflatedCap =
      yearsFrom2024 > 0
        ? Math.round(DEFAULT_SALARY_CAP * Math.pow(1.08, yearsFrom2024))
        : DEFAULT_SALARY_CAP;
    ...
    // Create player history for any new filler players generated by roster maintenance
    playerTeamMap = buildPlayerTeamMap(state.teams);
    for (const player of Object.values(state.players)) {
      if (!playerHistory[player.id]) {
        const teamId = findPlayerTeamFromMap(player.id, playerTeamMap) || '';
        ...
        if (teamId) {
          playerHistory[player.id] = addTransaction(playerHistory[player.id], {
            ...
            type: 'free_agent_signed',
            teamId,
            description: 'Signed as roster filler',
          });
        }
      }
    }

    // 6h. Enforce salary cap compliance - cut players to get under cap
    const capEnforcementResult = enforceCapCompliance(
      state.teams,
      state.players,
      state.contracts,
      currentYear + 1,
    ...
    state = {
      ...state,
      teams: capEnforcementResult.updatedTeams,
      players: capEnforcementResult.updatedPlayers,
      contracts: capEnforcementResult.updatedContracts,
    };

    // 6i. Second roster maintenance pass to refill after cap enforcement cuts
    const rosterResult2 = processRosterMaintenance(
      state.teams,
      state.players,
      state.contracts,
      currentYear + 1
    ...
    state = {
      ...state,
      teams: rosterResult2.updatedTeams,
      players: rosterResult2.updatedPlayers,
      contracts: rosterResult2.updatedContracts,
    };

    // Create player history for any new players from second roster fill
    playerTeamMap = buildPlayerTeamMap(state.teams);
    for (const player of Object.values(state.players)) {
      if (!playerHistory[player.id]) {
        const teamId = findPlayerTeamFromMap(player.id, playerTeamMap) || '';
    ...
    // 6j. Update team finances with inflated cap
    state = {
      ...state,
      teams: updateTeamFinances(state.teams, state.contracts, currentYear + 1, inflatedCap),
    };
  ...
  // Calculate the final year's salary cap (matching contract value inflation from 2024)
  const finalYearsFrom2024 = startYear - 2024;
  const finalCap =
    finalYearsFrom2024 > 0
      ? Math.round(DEFAULT_SALARY_CAP * Math.pow(1.08, finalYearsFrom2024))
      : DEFAULT_SALARY_CAP;

  // Final cap enforcement to ensure all teams start under cap
  const finalCapEnforcement = enforceCapCompliance(
    state.teams,
    state.players,
    state.contracts,
    startYear,
  ...
  state = {
    ...state,
    teams: finalCapEnforcement.updatedTeams,
    players: finalCapEnforcement.updatedPlayers,
    contracts: finalCapEnforcement.updatedContracts,
  };

  // Final finance update with the correct cap for the user's starting year
  state = {
    ...state,
    teams: updateTeamFinances(state.teams, state.contracts, startYear, finalCap),
  };
